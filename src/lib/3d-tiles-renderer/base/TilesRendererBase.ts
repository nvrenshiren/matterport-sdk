...
import { FAILED, LOADED, LOADING, PARSING, UNLOADED } from "./constants"
import { Tile } from "./Tile"
import { Tileset } from "./Tileset"
import { determineFrustumSet, markUsedSetLeaves, skipTraversal, toggleTiles, traverseSet } from "./traverseFunctions"
...
export class TilesRendererBase {
  public _autoDisableRendererCulling: boolean
  tileSets: Record<string, Tileset | Promise<void>>
  rootURL: string | null
  ...
  traverse(beforecb: Function | null = null, aftercb: Function | null = null) {
    const tileSets = this.tileSets
    const rootTileSet = tileSets[this.rootURL!] as Tileset
    if (!rootTileSet || !rootTileSet.root) return

    traverseSet(
      rootTileSet.root as Tile,
      (tile, ...args) => {
  ...
  update() {
    ...
    const root = rootTileSet.root as Tile
  ...
  preprocessNode(tile: Tile, tileSetDir, parentTile?: any) {
    if (tile.content) {
      ...
      if (
        ...
        !("box" in tile.content.boundingVolume || "sphere" in tile.content.boundingVolume || "region" in tile.content.boundingVolume)
      ) {
  ...
  ensureChildrenArePreprocessed(tile: Tile) {
    ...
    for (let i = 0, l = children.length; i < l; i++) {
      ...
      this.preprocessNode(child as Tile, tile.__basePath, tile)
    }
  }

  // Private Functions
  fetchTileSet(url: string, fetchOptions, parent = null) {
