...
function isUsedThisFrame(tile: Tile, frameCount) {
  return tile.__lastFrameVisited === frameCount && tile.__used
}

// Resets the frame frame information for the given tile
function resetFrameState(tile: Tile, frameCount) {
...
function recursivelyMarkUsed(tile: Tile, frameCount, lruCache, renderer) {
  ...
  if (tile.__contentEmpty) {
    const children = tile.children as Tile[]
...
function recursivelyLoadTiles(tile: Tile, renderer) {
  renderer.ensureChildrenArePreprocessed(tile)
...
export function traverseSet(tile: Tile, beforeCb: Function | null = null, afterCb: Function | null = null, parent?: any, depth?: number) {
  ...
  const children = tile.children as Tile[]
  for (let i = 0, l = children.length; i < l; i++) {
    traverseSet(children[i], beforeCb, afterCb, tile, (depth || 0) + 1)
  }
...
export function determineFrustumSet(tile: Tile, renderer) {
...
export function markUsedSetLeaves(tile: Tile, renderer) {
  const stats = renderer.stats
  const frameCount = renderer.frameCount
...
export function skipTraversal(tile: Tile, renderer) {
  const stats = renderer.stats
  const frameCount = renderer.frameCount
  if (!isUsedThisFrame(tile, frameCount)) {
    return
...
export function toggleTiles(tile: Tile, renderer) {
  const frameCount = renderer.frameCount
