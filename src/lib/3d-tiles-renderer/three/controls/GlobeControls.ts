import { Matrix4, Quaternion, Vector2, Vector3, MathUtils, Ray } from "three"
import { EnvironmentControls, NONE } from "./EnvironmentControls"
import {
  closestRayEllipsoidSurfacePointEstimate,
  closestRaySpherePointFromRotation,
  makeRotateAroundPoint,
  mouseToCoords,
...
export class GlobeControls extends EnvironmentControls {
  ...
  update() {
    ...
    const { camera, tilesGroup, pivotMesh } = this
  ...
  updateCameraClipPlanes(camera) {
    ...
    if (camera.isPerspectiveCamera) {
      const distanceToCenter = _vec.setFromMatrixPosition(tilesGroup.matrixWorld).sub(camera.position).length()
  ...
  _updatePosition() {
    if (this._dragMode === 1 || this._isNearControls()) {
      ...
      const { raycaster, camera, pivotPoint, pointerTracker, domElement, tilesGroup } = this
      ...
      const { pointerTracker, rotationSpeed, camera, pivotMesh, pivotPoint, tilesGroup } = this
      ...
      if (camera.isPerspectiveCamera) {
        ...
        scaleAmount = MathUtils.mapLinear(camera.zoom, this._getOrthographicTransitionZoom(), this._getMinOrthographicZoom(), 0.001, 0.005)
      }

      pointerTracker.getCenterPoint(_pointer)
      pointerTracker.getPreviousCenterPoint(_prevPointer)
      _deltaPointer.subVectors(_pointer, _prevPointer).multiplyScalar(scaleAmount)
