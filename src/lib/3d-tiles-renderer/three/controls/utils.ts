...
export function closestRaySpherePointFromRotation(ray, radius, target) {
  ...
  const rotationVec = _vec.crossVectors(target, ray.direction).normalize()
...
export function setRaycasterFromCamera(raycaster, mouse, camera) {
  ...
  origin.set(mouse.x, mouse.y, -1).unproject(camera)

  direction.set(mouse.x, mouse.y, 0).unproject(camera).sub(origin).normalize()
