import { Matrix4, Quaternion, Vector2, Vector3, Raycaster, Plane, EventDispatcher, MathUtils } from "three"
import { PivotPointMesh } from "./PivotPointMesh"
import { PointerTracker } from "./PointerTracker"
import { mouseToCoords, makeRotateAroundPoint, setRaycasterFromCamera } from "./utils"
...
export class EnvironmentControls extends EventDispatcher {
  ...
  attach(domElement) {
    ...
    const pointerdownCallback = e => {
      ...
      const { camera, raycaster, domElement, up, pivotMesh, pointerTracker } = this
  ...
  update() {
    ...
    const { camera, cameraRadius, pivotPoint, up, state, adjustHeight } = this
  ...
  // private
  _updateZoom() {
    const { zoomPoint, zoomDirection, camera, minDistance, maxDistance, raycaster, pointerTracker, domElement, minZoom, maxZoom, zoomSpeed } = this
  ...
  _updateZoomPoint() {
    const { camera, zoomDirectionSet, zoomDirection, raycaster, zoomPoint, pointerTracker, domElement } = this
  ...
  _updatePosition() {
    const { raycaster, camera, pivotPoint, up, pointerTracker, domElement } = this
  ...
  _updateRotation() {
    const { camera, pivotPoint, minAltitude, maxAltitude, pointerTracker, rotationSpeed } = this
  ...
  _setFrame(newUp, pivot) {
    const { up, camera, state, zoomPoint, zoomDirection, zoomDirectionSet, zoomPointSet, reorientOnDrag, scaleZoomOrientationAtEdges } = this
